...
BTN_KEEP_RECORD = "Оставить запись"

RECORD_PAGE_SIZE = 6


def build_records_keyboard(labels: list[str], page: int = 0) -> str:
    chunk, _, has_prev, has_next = paginate(labels, page, RECORD_PAGE_SIZE)
...
def build_cancel_confirm_keyboard() -> str:
    keyboard = VkKeyboard(one_time=True)
    keyboard.add_button(BTN_CONFIRM_CANCEL, color=VkKeyboardColor.NEGATIVE)
    keyboard.add_line()
    keyboard.add_button(BTN_KEEP_RECORD, color=VkKeyboardColor.SECONDARY)
    return keyboard.get_keyboard()
